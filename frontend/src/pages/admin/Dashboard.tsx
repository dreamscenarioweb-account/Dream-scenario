...
import { fetchAdminAlbums, fetchQuoteRequests } from "@/lib/adminApi";
import type { Album, QuoteRequest } from "@/types";
...
const Dashboard = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const [albums, quotes] = await Promise.all([
        ...
        setStats({
          ...
          published: albumData.filter((a: Album) => a.is_published).length,
          drafts: albumData.filter((a: Album) => !a.is_published).length,
          unreadQuotes: quoteData.filter((q: QuoteRequest) => !q.is_read).length,
        });
      } finally {
        setLoading(false);
      }
    ...
    <div className="space-y-12 pb-12">
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-wrap gap-4">
          ...
          <Link to="/admin/albums?action=new" className="flex items-center gap-2 px-5 py-2.5 bg-[#0f172a] text-white rounded-full hover:bg-black transition-colors font-body text-sm font-medium shadow-[0_2px_10px_rgba(0,0,0,0.1)]">
